...
export function normalizeReviewerOutput(output, { round, priorReviews, overrides }) {
  ...
  for (const disposition of output.previousFindings) {
    if (disposition.status === 'severity_changed') {
      if (!disposition.effectiveSeverity) throw new Error(`${disposition.id} severity_changed needs effectiveSeverity`);
    } else if (disposition.effectiveSeverity !== null) {
      throw new Error(`${disposition.id} effectiveSeverity must be null unless status is severity_changed`);
    }
  ...
  const normalized = { ...output, newFindings };
  const synthetic = {
    meta: { round },
    review: normalized
  };
  ...
  return { normalized, findings: after, requiredBefore: required };
}
