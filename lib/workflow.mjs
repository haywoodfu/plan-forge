...
async function finalize(context, logger = NOOP_LOGGER) {
  ...
  if (!approval) {
    ...
    approval = {
      ...
      publishedPath: path.relative(context.paths.repoRoot, context.paths.published),
      approvedAt: now()
    };
  ...
  // the workflow's single sanctioned write outside .ai/plan-reviews/.
  const provenance = `<!-- plan-review: task=${context.task.taskId} round=${approval.round} author=${context.task.author} reviewer=${context.task.reviewer} approvedAt=${approval.approvedAt} planSha256=${approval.planSha256} -->\n\n`;
  const publishedContent = provenance + author.plan;
  if (await readTextIfExists(context.paths.published) !== publishedContent) {
    await atomicWriteFile(context.paths.published, publishedContent, { mode: 0o644 });
  }
  logger.stage('final plan published', { phase: 'finalizing', round: review.meta.round, file: context.paths.published });
...
export async function inspectTask({ repoRoot, taskId, schemas }) {
  ...
  if (context.approval) {
    return {
      ...
      publishedPath: context.paths.published
    };
  }
