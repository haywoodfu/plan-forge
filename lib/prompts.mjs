...
// The author's view adds attribution: it needs raisedBy to judge which
// findings are the same defect (coversFindingIds), and AC5 wants the trace.
// Slot ids only — never provider or model names, which would invite dismissing
// findings by model reputation. The ?? 'R1' default exists solely for findings
// folded from v1 (legacy) rounds, which predate raisedBy; every v2 merged
// finding carries the field, so a v2 round can never reach the default.
function authorFinding(finding) {
  return { ...sanitizedFinding(finding), raisedBy: finding.raisedBy ?? 'R1' };
}
...
export function buildAuthorPrompt({ templates, agentsMd, requirement, previousPlan, findings, overrides }) {
  ...
  parts.push(block('ACTIVE FINDINGS', JSON.stringify(findings.map(authorFinding), null, 2)));
